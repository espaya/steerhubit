    ...
    <div class="dashboard__content d-flex">
        ...
        <div class="dashboard__right">
            <div class="dash__content ">
                ...
                <!-- sidebar menu end -->
                @if(session('success'))
                    <div style="background-color:green; color:lightgreen; padding-left:20px; padding-top:20px; padding-bottom:20px"> 
                        {{ session('success') }} 
                    </div>
                @endif

                @if(session('error'))
                <div style="background-color:red; color:black; padding-left:20px; padding-top:20px; padding-bottom:20px">
                    {{ session('error') }}
                </div>
                @endif
                    ...
                    <div class="applied__job__list">
                        ...
                        <div class="single__applied__job">
                            <div class="single__applied__job__content">
                                <div class="icon">
                                    <img src="{{ Auth::check() && Auth::user()->avatar ? asset('uploads/avatars/' . Auth::user()->avatar) : asset('assets/img/dashboard/profile.png') }}" alt="">
                                </div>
                                <div class="content">
                                    <a href="#">
                                        <h6>{{ $job->title }}</h6>
                            ...
                            <div class="middle__content ">
                                <div class="job__post">
                                    ...
                                    <div class="content">
                                        <span>Job Posted: {{ $job->created_at->diffForHumans() }}</span>
                                        <span>Job Deadline: {{ \Carbon\Carbon::parse($job->deadline)->format('F d, Y') }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="action"> 
                                <a href="{{ route('employer.job.edit', ['slug' => $job->slug]) }}" class="action__btn">
                                ...
                                <a href="{{ route('job.view', ['slug' => $job->slug]) }}">
                                    @csrf
                    ...
                    @if ($jobs->hasPages())
                        <div class="rts__pagination d-block mx-auto pt-40 max-content">
                            <ul class="d-flex gap-2">
                                ...
                                {{-- Previous Page Link --}}
                                @if ($jobs->onFirstPage())
                                    <li><a class="inactive"><i class="rt-chevron-left"></i></a></li>
                                @else
                                ...
                                @foreach ($jobs->getUrlRange(1, $jobs->lastPage()) as $page => $url)
                                    <li>
                                        <a href="{{ $url }}" class="{{ $page == $jobs->currentPage() ? 'active' : '' }}">{{ $page }}</a>
                                    </li>
                                @endforeach
                                ...
                                @else
                                    <li><a class="inactive"><i class="rt-chevron-right"></i></a></li>
                                @endif
